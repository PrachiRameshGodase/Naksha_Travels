...
const DropDownHelper = (options, onChange, name, type, setItemData, setcusData, setBasicDetailsDisplayName) => {
    ...
    const handleSelect = (option) => {
        if (option.active !== "0") {
            ...
            onChange({
                target: {
                    name,
                    value: type === "masters" ? option.labelid : type === "taxRate" ? option?.tax_percentge : type === "currency" ? option?.code : type === "rate" ? option : type === "masters_salutation" ? option?.label : type === "account" ? option?.account_type : option.id,
    ...
    useEffect(() => {
        if (searchTerm?.length >= 1) {
            DropdownSearchHealperfunctions(searchTerm, type, dispatch);
        }
    }, [searchTerm]);
