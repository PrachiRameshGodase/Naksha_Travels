...
const EditQuotation = () => {
    ...
    // console.log("formdata?.sale_type", formData)

    const storedItems = fetchQuotations?.items?.map(item => ({
        quantity: (+ item.quantity),
        gross_amount: (+item.gross_amount),
        discount: (+item.discount_after_tax),
    ...
    const storedItemsId = fetchQuotations?.items?.map(item => ({
        item: item
    }));
    ...
    useEffect(() => {
        setFormData({
            ...
            expiry_date: fetchQuotations?.expiry_date,
            customer_name: fetchQuotations?.customer?.name,
            customer_id: fetchQuotations?.customer?.id,
            email: fetchQuotations?.customer?.email,
            customer_type: fetchQuotations?.customer?.type,
            phone: fetchQuotations?.customer?.phone,
            terms: fetchQuotations?.terms,
            customer_note: fetchQuotations?.customer_note,
            place_of_supply: fetchQuotations?.customer?.place_of_supply,
            // address: fetchQuotations?.customer?.address,
            items: storedItems,
        });
        // setItems(finalItems);
    }, [customers, fetchQuotations?.customer?.place_of_supply, fetchQuotations?.customer_note, fetchQuotations?.terms, fetchQuotations?.quotation_id, fetchQuotations?.fetchQuotations, fetchQuotations?.sale_person, fetchQuotations?.reference_no, fetchQuotations?.transaction_date, fetchQuotations?.expiry_date, fetchQuotations?.customer?.name, fetchQuotations?.customer?.phone, fetchQuotations?.customer?.type,]);
    ...
    const handleClicktoConvertData = async (val) => {
        try {
            if (val === "convert-sales-order") {
                const { quotation_id, id, ...formDataWithoutQuotationId } = formData;
                setFormData({
                    ...formDataWithoutQuotationId,
                    sale_type: "sale_order",
                    sale_order_id: formData.quotation_id,
                    payment_terms: "2",
                    id: 0,
            ...
            if (val === "convert-invoices") {
                const { quotation_id, ...formDataWithoutQuotationId } = formData;
                setFormData({
                    ...formDataWithoutQuotationId,
                    sale_type: "invoice",
                    invoice_id: formData.quotation_id,
                    payment_terms: "2",
                    id: 0,
                });
                localStorage.setItem("ConFormData", JSON.stringify({
    ...
    const fetchAllQuatation = async () => {
        try {
            const getData = await axiosInstance.post(`/quotations/details?id=${id}`, {
            });
            setFetchQuotations(getData?.data?.quotation)
        } catch (e) {
                    ...
                    <div id='parentnewformmodern' onKeyDown={handleKeyDown}>
                        ...
                        <div id="submitbuttonrow">
                            <div className="form-grscsdoup">
                                {convert === "convert-invoices" || convert === "convert-sales-order" ?
                                    <button type="submit" onClick={() => handleClicktoConvertData(convert)}>
