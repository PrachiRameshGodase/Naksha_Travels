...
const CreateWarehouse = () => {
    ...
    const allDepartment = ShowMasterData("10")
    ...
    useEffect(() => {
        if ((itemId && isEdit && warehouseDetail) || (itemId && isDuplicate && warehouseDetail) || itemId && (convert === "saleToInvoice")) {
    ...
    }, [itemId, isEdit, convert, warehouseDetail, isDuplicate]);
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            await dispatch(warehouseCreateAction(sendData, Navigate, isEdit));
        } catch (error) {
    ...
    return (
        <>
            <Toaster />
            <TopLoadbar />
            {/* {billDetailss?.loading || warehouseDetails?.loading ? <Loader02 /> : <> */}
            {loading && <MainScreenFreezeLoader />}
            {states?.loading && <MainScreenFreezeLoader />}
            {cities?.loading && <MainScreenFreezeLoader />}
            {warehouseCreate?.loading && <MainScreenFreezeLoader />}
