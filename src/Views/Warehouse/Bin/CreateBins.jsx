...
import { binCreateAction, binDetailAction, binViewAction, rackCreateAction, rackViewAction, warehouseCreateAction, warehouseDetailAction, warehouseViewAction, zoneCreateAction, zoneViewAction } from '../../../Redux/Actions/warehouseActions';
...
const CreateBins = () => {
    ...
    const binCreate = useSelector(state => state?.binCreate);

    const warehouseView = useSelector(state => state?.warehouseView);
    const warehouseViews = warehouseView?.data;

    const binDetails = useSelector(state => state?.binDetail);
    const binDetailss = binDetails?.data?.data;
    ...
    useEffect(() => {
        if ((itemId && isEdit && binDetailss) || (itemId && isDuplicate && binDetailss) || itemId && (convert === "saleToInvoice")) {
            setFormData({
                id: isEdit ? itemId : 0,
                warehouse_id: (+binDetailss?.warehouse_id),
                zone_id: (+binDetailss?.zone_id),
                rack_id: (+binDetailss?.rack_id),
                name: binDetailss?.name,
                description: (binDetailss?.description),
                // capacity: binDetailss?.capacity,

            });
        }
    }, [itemId, isEdit, convert, binDetailss, isDuplicate]);
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            await dispatch(binCreateAction(formData, Navigate));
        } catch (error) {
    ...
    useEffect(() => {
        dispatch(binDetailAction({ bin_id: itemId }));
    ...
    return (
        <>
            <Toaster />
            <TopLoadbar />
            {/* {billDetailss?.loading || binDetailsss?.loading ? <Loader02 /> : <> */}
            {loading && <MainScreenFreezeLoader />}
            {binCreate?.loading && <MainScreenFreezeLoader />}
                        ...
                        <div className="relateivdiv">
                            ...
                            <div className="itemsformwrap">
                                <div className="f1wrapofcreq">
                                    ...
                                    <div className="f1wrapofcreqx1">
                                        ...
                                        <div className="form_commonblock ">
                                            ...
                                            <span >
                                                {otherIcons.placeofsupply_svg}
                                                <CustomDropdown03
                                                    options={zoneViews?.filter(val => val?.is_active == 1) || []}
                                        ...
                                        <div className="form_commonblock ">
                                            <label className='color_red'>Racks</label>
                                            <span >
                                                {otherIcons.placeofsupply_svg}
                                                <CustomDropdown03
                                                    options={rackViews?.filter(val => val?.is_active == 1) || []}
                                    ...
                                    <div className="f1wrapofcreqx1">
                                        <div className="form_commonblock">
                                            <label className='color_red'>Bin Name</label>
                                            <span >
                                                {otherIcons.placeofsupply_svg}
                                                <input
                                                    type="text"
                                        ...
                                        {/* <div className="form_commonblock">
                                            <label className='color_red'>Bin Code</label>
                                            <span >
                                                {otherIcons.placeofsupply_svg}
                                                <input
                                                    type="text"
                                        ...
                                        {/* <div className="form_commonblock ">
                                            <label className='color_red'>Capacity</label>
                                            <span>
                                                {otherIcons.placeofsupply_svg}
                                                <input
