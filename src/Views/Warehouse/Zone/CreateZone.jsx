...
import { warehouseDetailAction, warehouseViewAction, zoneCreateAction, zoneDetailAction } from '../../../Redux/Actions/warehouseActions';
...
// import { zone_type } from '../../Helper/ComponentHelper/DropdownData';
...
const CreateZone = () => {
    ...
    const zoneCrate = useSelector(state => state?.zoneCrate);

    const item_details = useSelector(state => state?.zoneDetail || {});
    const zoneView = item_details?.data?.data;
    ...
    useEffect(() => {
        if ((itemId && isEdit && zoneView) || (itemId && isDuplicate && zoneView) || itemId && (convert === "saleToInvoice")) {
            setFormData({
                id: isEdit ? itemId : 0,
                warehouse_id: (+zoneView?.warehouse_id),
                zone_type: zoneView?.zone_type,
                name: zoneView?.name,
                zone_no: (zoneView?.zone_no),
                description: zoneView?.description,
                no_of_aisle: (zoneView?.no_of_aisle),
                no_of_racks: zoneView?.no_of_racks,
                level: (zoneView?.level),
            });

        }
    }, [itemId, isEdit, convert, zoneView, isDuplicate]);
    ...
    const handleChange = (e) => {
        ...
        if (name === "warehouse_id") {
            const options = warehouseViews?.find(val => val?.id == value);
            setItemData(options);

            setFormData({
                ...formData,
                zone_type: options?.warehouse_type,
                [name]: newValue,
            });
        } else {
            setFormData({
    ...
    const handleFormSubmit = async (e) => {
        ...
        try {
            ...
            await dispatch(zoneCreateAction(formData, Navigate));
        } catch (error) {
    ...
    useEffect(() => {
        dispatch(zoneDetailAction({ zone_id: itemId }));
    ...
    return (
        <>
            <Toaster />
            <TopLoadbar />
            {/* {billDetailss?.loading || zoneViews?.loading ? <Loader02 /> : <> */}
            {loading && <MainScreenFreezeLoader />}
            {zoneCrate?.loading && <MainScreenFreezeLoader />}
            {/* {countries?.loading && <MainScreenFreezeLoader />} */}
                        ...
                        <div className="relateivdiv">
                            ...
                            <div className="itemsformwrap">
                                <div className="f1wrapofcreq">
                                        ...
                                        <div className="form_commonblock ">
                                            ...
                                            <span >
                                                ...
                                                <CustomDropdown03
                                                    ...
                                                    name="warehouse_id"
                                                    defaultOption="Select Warehouse Name"
                                                    type="categories"
                                                />
                                            </span>
                                        </div>
                                        <div className="form_commonblock ">
                                            <label className='color_red'>Zone Type</label>
                                            <span >
                                                {otherIcons.placeofsupply_svg}
                                                <input
                                                    type="text"
                                                    ...
                                                    onChange={handleChange}
                                                    name='zone_type'
                                                    placeholder='Select Warehouse For Type'
                                                    readOnly
                                                />
                                            </span>
                                        </div>
                                    ...
                                    <div className="f1wrapofcreqx1">
                                        <div className="form_commonblock">
                                            <label className='color_red'>{itemData?.warehouse_type === "Container" ? "Container" : "Zone"} Name</label>
                                            <span >
                                                {otherIcons.placeofsupply_svg}
                                                <input
                                                    type="text"
                                        ...
                                        <div className="form_commonblock">
                                            <label >No Of Rack/Shelf</label>
                                            <span>
                                                {otherIcons.placeofsupply_svg}
                                                <input
                                                    value={formData.no_of_racks}
                                                    onChange={handleChange}
                                                    name="no_of_racks"
                                                    placeholder="Enter No Of Rack/Shelf"
                                                />
                                            </span>
                                        </div>
                                        <div className="form_commonblock">
                            ...
                            <div className="actionbarcommon">
                                ...
                                <button className="firstbtnc1" type="submit">{isEdit ? "Update" : "Save"}
                                ...
                                <Link to={"/dashboard/warehouse"} className="firstbtnc2">
                                    Cancel
                                </Link>
                            </div>
